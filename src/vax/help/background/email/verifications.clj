...
(defn send-verification-email
  ...
  (try
    (let [msg           (sub->email sub (cv :baseurl))
          ...
          result        (do
                          (μ/log ::sending-email, :subscription/id id, :subscription/nonce nonce
                                                  :email/from (.getFrom msg), :email/to (.getTo msg)
    ...
    (catch Exception e
      (μ/log ::postmark-error, :subscription/id id, :ex-class (.getSimpleName e), :ex-msg (.getMessage e), :ex-data (ex-data e))
